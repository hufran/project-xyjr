...
function parseList(date){
    for(var i=0;i<date.length;i++){
        ...
        date[i].description=(/^[\u4e00-\u9fa5\;\:\：\，\。\,\.0-9]+$/.test(date[i].description))?date[i].description:(!date[i].description?"":"失败");
        date[i].hint=(/^[\u4e00-\u9fa5\;\:\：\，\。\,\.0-9]+$/.test(date[i].hint))?date[i].hint:(!date[i].hint?"":"失败");
    }
    return date;
}
